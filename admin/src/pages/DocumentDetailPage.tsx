	...
	return (
		<div className="mx-auto max-w-3xl">
			...
			<div className="mb-6 flex items-center justify-between">
				<h1 className="text-2xl font-semibold text-cream">{attachment.original_filename}</h1>
				<a
			...
			<div className="mb-6 rounded-lg border border-line bg-panel p-6">
				...
				<dl className="grid grid-cols-2 gap-y-2 text-sm">
					...
					<dd className="text-cream/80">{getDocumentCategory(attachment.original_filename)}</dd>
					<dt className="text-cream/50">Dodano</dt>
					<dd className="text-cream/80">{new Date(attachment.created_at).toLocaleDateString('pl-PL')}</dd>
				</dl>
			</div>

			<div className="mb-6 rounded-lg border border-line bg-panel p-6">
				<div className="mb-3 flex items-center justify-between">
					<h2 className="text-sm font-medium text-cream">Powiązane maszyny ({linkedDevices?.length ?? 0})</h2>
					<button onClick={() => setShowAssignPanel((v) => !v)} className="text-sm text-ember hover:underline">
					...
					<div className="max-h-80 space-y-1 overflow-y-auto">
						{allDevices?.map((device) => {
							...
							return (
								...
								>
									<input type="checkbox" checked={linked} onChange={() => void toggleDevice(device.id, linked)} />
									{device.name}
								</label>
							)
						})}
			...
			<div className="rounded-lg border border-red-900/40 bg-panel p-6">
				...
				<p className="mb-3 text-sm text-cream/50">Usunięcie dokumentu jest nieodwracalne.</p>
				<button
