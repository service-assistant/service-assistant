	...
	return (
		<div className="mx-auto max-w-2xl">
			...
			<div className="mb-6 space-y-4 rounded-lg border border-line bg-panel p-6">
				<div>
					<label className="mb-1 block text-xs uppercase tracking-wide text-cream/50">Nazwa modelu</label>
					<input
						value={name}
				...
				<div>
					<label className="mb-1 block text-xs uppercase tracking-wide text-cream/50">Kod modelu (opcjonalnie)</label>
					<input
				...
				<div>
					<label className="mb-1 block text-xs uppercase tracking-wide text-cream/50">Marka</label>
					<select
						value={brandId ?? ''}
				...
				<div>
					<label className="mb-1 block text-xs uppercase tracking-wide text-cream/50">Typ maszyny</label>
					<select
						value={deviceTypeId ?? ''}
						onChange={(e) => setDeviceTypeId(e.target.value ? Number(e.target.value) : null)}
						className="w-full rounded-md border border-line bg-panel-soft px-3 py-2 text-sm text-cream"
					>
				...
				<div>
					<label className="mb-1 block text-xs uppercase tracking-wide text-cream/50">URL zdjęcia (opcjonalnie)</label>
					<input
						value={imageUrl}
						onChange={(e) => setImageUrl(e.target.value)}
						className="w-full rounded-md border border-line bg-panel-soft px-3 py-2 text-sm text-cream outline-none focus:border-ember"
					/>
					{imageUrl && <img src={imageUrl} alt="" className="mt-2 h-24 rounded object-contain" />}
				</div>
			</div>

			<div className="mb-6 rounded-lg border border-line bg-panel p-6">
				<div className="mb-3 flex items-center justify-between">
					<label className="text-xs uppercase tracking-wide text-cream/50">Przypisz istniejące dokumenty</label>
					<span className="text-xs text-cream/50">{selectedLabel(selectedAttachmentIds.length)}</span>
				</div>
